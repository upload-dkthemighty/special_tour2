...
const BG_MOVING_SPEED = 6; // 배경 이동 속도
let bgX = 0; // 배경 X 좌표
...
startImage.src = "./images/background.png";
// (3) 게임 오버
...
rtanAImage.src = "./images/rtan_running_a1.png";
// (6) 달리는 르탄이 B
const rtanBImage = new Image();
rtanBImage.src = "./images/rtan_running_a1.png";
// (7) 게임 오버 르탄이
const rtanCrashImage = new Image();
rtanCrashImage.src = "./images/rtan_crash2.jpg";
// (8) 장애물
...
function animate() {
    ...
    // 1-3 르탄이 점프 조건 설정하기
    if (jump) {
        rtan.y -= jumpSpeed; // 스페이스바를 누르고 있으면 rtan의 y값 감소
        if (rtan.y < 20) rtan.y = 20; // rtan이 canvas 상단을 넘지 않도록 조정
    } else {
        if (rtan.y < RTAN_Y) {
            rtan.y += jumpSpeed; // 스페이스바를 떼면 rtan의 y값 증가
